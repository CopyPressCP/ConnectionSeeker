...
class CActiveDataProvider extends CDataProvider
{
	...
	protected function fetchData()
	{
		...
		if(($pagination=$this->getPagination())!==false)
		{
            ...
            //## Added by Leo 2/4/2013, Please reference the comments of the $speedUpByIn declare ######
            if ($this->speedUpByIn == true) {
                $baseCriteria=$this->model->getDbCriteria(false);
                $cc = clone $criteria;
                $bc = clone $baseCriteria;
                $pagination->applyLimit($cc);
                $bc->mergeWith($cc);
                ...
                //$command = $this->model->getCommandBuilder()->createFindCommand($this->model->getTableSchema(), $bc);
                //echo $sql = $command->getText();
                //$criteria->addCondition("t.id IN (SELECT m.id FROM ($sql) AS m)");

                $inrs = $this->model->findAll($bc);
                $inarr = array();
                if ($inrs) {
                    foreach ($inrs as $_r) {
                        $inarr[] = $_r->id;
                ...
                //print_r($inarr);
                if (!empty($inarr)) {
                    $criteria->addCondition("t.id IN (".implode(",", $inarr).")");
                } else {
                    $criteria->addCondition("t.id IS NULL");
                }
            } else {
                $pagination->applyLimit($criteria);
            }
            //## Added by Leo 2/4/2013 ######
		}
		...
		if(($sort=$this->getSort())!==false)
		{
			...
			if ($this->speedUpByIn != true) $sort->applyOrder($criteria);
		}
