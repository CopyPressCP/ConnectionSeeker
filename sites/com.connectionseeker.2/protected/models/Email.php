...
class Email extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('domain_id, template_id, from, cc, status, created_by, modified_by, parent_id, is_reply', 'numerical', 'integerOnly'=>true),
			array('to', 'length', 'max'=>255),
			array('subject', 'length', 'max'=>1000),
			array('content, send_time, created, modified, email_from, mid', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, domain_id, mid, template_id, from, to, cc, subject, content, status, send_time, created, created_by, modified, modified_by', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
        ...
        if (is_numeric($this->domain_id)) {
            ...
            $dm = Domain::model()->find('domain=:domain',array(':domain'=>$this->domain_id));
            if ($dm) {
        ...
        if (isset($this->parent_id)) {
            ...
            $criteria->addCondition('(parent_id = 0) OR (parent_id IS NULL)');
        }
