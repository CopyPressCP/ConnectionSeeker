...
class TabController extends MController
{
	...
	*/
    ...
    public function actionMenus()
    {
        ...
        Yii::app()->user->rightsReturnUrl =$url;
        $params = array(
			'parent_id' => $pid,
			'sortable'=>array(
	...
	{
        Yii::app()->user->rightsReturnUrl = array('tab/index');
    ...
    public function actionGenerate()
    {
        ...
        $menuItems = $this->_menu->getAllMenus();
       ...
       $formModel = new MenuForm('update');
		...
		if( isset($_POST['Menu'])===true )
		{
			...
			if( $formModel->validate()===true )
			{
				...
				$this->redirect(Yii::app()->user->rightsReturnUrl);
			}
		}
	...
	public function actionCreate()
	{
		...
		if( isset($_POST['MenuForm'])===true )
		{
			...
			if( $formModel->validate()===true )
			{
                ...
                $formModel->is_tab = $pid ? 0 :1;
                if ($pid > 0) $formModel->parent_id = $pid;
				// Create the item
                $model->attributes = $formModel->attributes;
				$model->save();
                //print_r($model);
				...
				// Redirect to the correct destination
				$this->redirect(Yii::app()->user->getRightsReturnUrl(array('tab/menus')));
			}
		}
