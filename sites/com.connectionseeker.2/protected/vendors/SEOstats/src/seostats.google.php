...
class SEOstats_Google extends SEOstats {
    ...
    */
    ...
    public static function Google_PR($q,$host='toolbarqueries.google.com',$context=NULL) 
    {
        //echo $q;
        ...
        //echo $url=sprintf($url,$host,$ch,$q);
        ...
        $pr = str_replace("\n", "", $pr);
        $pr = $pr?substr(strrchr($pr, ':'), 1):false;

	    $valid_response_codes = array('200','301','302');
        if(in_array($status_code, $valid_response_codes)){
            if ($pr === false) $pr = -2;
        }
        //pr = -2 means unrank, pr = -1 means need to re-check
