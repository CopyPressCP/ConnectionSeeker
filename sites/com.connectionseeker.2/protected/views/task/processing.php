...
if ($displaymode == 1) {//pre-content
    $dparr = array("progressstatus","tasktype","anchortext","targeturl","rewritten_title","blog_title","blog_url","notes");
} elseif ($displaymode == 2) {//QA Stuff
    $dparr = array("progressstatus","tasktype","anchortext","targeturl","tierlevel","sourceurl","rewritten_title","qa_comments");
} elseif ($displaymode == 3) {//Outreach Stuff
    $dparr = array("progressstatus","tasktype","anchortext","tierlevel","desired_domain_id","channel_id","sourceurl","rewritten_title","notes","qa_comments");
} else {//display all of these stuff;
    $dparr = "ALL";
}
*/
...
<?php $this->widget('application.extensions.lkgrid.LinkmeGridView', array(
	...
	'columns'=>array(
        ...
        array(
            'name' => 'progressstatus',
            'type' => 'raw',
            'value' => 'CHtml::dropDownList("progressstatus[]", $data->progressstatus, '.$pgstatusstr.')',
            'filter' => $pgstatus,
        ...
        */
        ...
        array(
            ...
            'value' => 'CHtml::textField("duedate[]", $data->duedate ? date("Y-m-d",$data->duedate) : "", array("id"=>"duedate_".$data->id, "readOnly"=>"readOnly"))',
            ...
            'buttons' => array(
                ...
                */
                'ioready' => array(
                    ...
                    'imageUrl'=>Yii::app()->theme->baseUrl.'/css/gridview/ioready.png',
...
$(document).ready(function() {
    ...
    $.fn.yiiGridView.defaults.afterAjaxUpdate = function(){
        ...
        $("select[name^='progressstatus'],select[name^='tasktype'],select[name^='tierlevel'],select[name^='desired_domain_id'],input[name^='livedate'],input[name^='duedate']").each(function() {
            //way 1
            ...
            if ($(this).attr('name') == "desired_domain_id[]"){
                ...
                //hidden "Pending/In Use" from the current text
            ...
            $(this).unbind('click').change(function(){
                ...
                $.ajax({
                    ...
                    'success':function(data){
                        //donothing for now;
                        if (data.success){
                            if (data.status && $(thistd).attr('name') == "desired_domain_id[]"){
                                $(thistd).parent().parent().find('select[name^="progressstatus"]').val(data.status);
                            }
        ...
        $("input[name^='rewritten_title'],input[name^='sourceurl'],input[name^='blog_title'],input[name^='blog_url'],input[name^='blog_url'],input[name^='target_stype'],textarea[name^='qa_comments'],textarea[name^='notes'],textarea[name^='client_comments']").each(function() {
