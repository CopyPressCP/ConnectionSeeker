...
class ClientDomainCompetitor extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('client_id, competitor_id', 'required'),
			array('client_id, competitor_id, fresh_called, historic_called, created_by, modified_by', 'numerical', 'integerOnly'=>true),
			array('domain_id', 'length', 'max'=>255),
			array('created, modified', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, domain_id, client_id, competitor_id, fresh_called, historic_called, created, created_by, modified, modified_by', 'safe', 'on'=>'search'),
		);
	}
