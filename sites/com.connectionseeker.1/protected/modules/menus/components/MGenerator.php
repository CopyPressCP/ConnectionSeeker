<?php
/**
* Menus generator component class file.
*
		...
		try {
			...
			foreach( $this->_items as $type=>$items )
			{
				...
				foreach( $items as $name )
				{
					...
					if( $menu->getMenuItem($name)===null )
					{         
                        $url = Menus::module()->getMenu()->generateUrl($name);
						// Insert item
						$sql = "INSERT INTO {$itemTable} (name, itemname, type, url)
							VALUES (:name, :itemname, :type, :url)";
	...
	*/
	protected function getControllersInModules($path)
	{
		...
		if( file_exists($modulePath)===true )
		{
			...
			{
				if( substr($entry, 0, 1)!=='.' && $entry!=='menus' )
				{
