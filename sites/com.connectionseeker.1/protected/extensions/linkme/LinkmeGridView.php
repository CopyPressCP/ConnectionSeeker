...
/**
 * LinkGridView displays a list of data items in terms of a table.
 *
 ...
 *
 * $this->widget('application.extensions.linkme.LinkmeGridView', array(
 ...
 *        'name' => 'id',
 *        'htmlOptions' => array('class'=> 'checkboxcont'),
 *        'checkBoxHtmlOptions' => array('name' => 'abid[]', 'id'=> 'abid', 'onclick' => 'alert(this.value);'),
 *    ),
 *    'isAddCheckBox' => true, // default is ture
 *     'columns'=>array(
 ...
 * @since 1.1
 */
class LinkmeGridView extends CBaseListView
{
	...
	protected function initColumns()
	{
		...
		$id=$this->getId();
        $this->totalCol = count($this->columns);
		foreach($this->columns as $i=>$column)
		{
            ...
            if ($i == 0) {
                $column->htmlOptions = array('class' => 'firstcont');
                $column->headerHtmlOptions = array('class' => 'firstcont');
                //$column->footerHtmlOptions = array('class' => 'footercont');
            } else if (($i+1) == $this->totalCol) {
                $column->htmlOptions = array('class' => 'lastcont');
                $column->headerHtmlOptions = array('class' => 'lastcont');
                //$column->footerHtmlOptions = array('class' => 'footercont');
            }
			if(!$column->visible)
			{
...
//				$this->renderFilter();
	...
	public function renderTableBody()
	{
		...
		{
			for($row=0;$row<$n;++$row)
				$this->renderTableRow($row, $n);
		}
		else
		{
			echo '<tr>' . ($this->isAddCheckBox ? '<td class="noncol" ></td>' : '') . '<td colspan="'.count($this->columns).'" class="noncol"  >';
			$this->renderEmptyText();
			echo "</td></tr>\n";
		}
		echo "</tbody>\n";
	 ...
	 */
	public function renderTableRow($row, $total)
	{
        $lastClass = (($row+1) == $total) ? ' footercont' : '';
		if($this->rowCssClassExpression!==null)
		{
			$data=$this->dataProvider->data[$row];
			echo '<tr class="'.$this->evaluateExpression($this->rowCssClassExpression,array('row'=>$row,'data'=>$data)). $lastClass . '">';
		}
		else if(is_array($this->rowCssClass) && ($n=count($this->rowCssClass))>0)
			echo '<tr class="'.$this->rowCssClass[$row%$n]. $lastClass . '">';
		else
			echo '<tr>';
        if ($this->isAddCheckBox) {
            $column = $this->checkboxColumns;
            if (!empty($this->checkboxColumns)) {
                if (is_string($this->checkboxColumns)) {
                    $column = array('name' => $this->checkboxColumns);
                }
            } else {
                $column = array('name' => $this->columns[0]->name);
                if (!isset($column['checkBoxHtmlOptions'])) {
                    $column['checkBoxHtmlOptions'] = array();
                }
                if (!isset($column['checkBoxHtmlOptions']['name'])) {
                    $column['checkBoxHtmlOptions']['name'] = 'ck' . $column['name'].'[]';
                }
            }
            if (!isset($column['class'])) $column['class'] = 'zii.widgets.grid.CCheckBoxColumn';
            if (!isset($column['htmlOptions'])) $column['htmlOptions'] = array();
            if (!isset($column['htmlOptions']['class'])) $column['htmlOptions']['class'] = 'checkboxcont';
