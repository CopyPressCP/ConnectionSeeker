    ...
    public function run($args) {
        ...
        */
        ...
        $competitor = Yii::app()->db->createCommand()
            ...
            ->join('{{client}} cl', 'cd.client_id=cl.id')
            ->where(
                array('and',
                    'cl.status = 1',
                    'cd.status = 1',
                    array('or', array('and', 'cd.use_historic_index=1', 
        ...
        */
        ...
        $mjurl = "http://enterprise.majesticseo.com/api_command?app_api_key=F4823AB4B88259A221E162F9865986E7&cmd=GetTopBackLinks&MaxSourceURLs=2000&GetRootDomainData=1&AnalysisResUnits=10000&ShowDomainInfo=1&GetUrlData=0&UseResUnits=1&URL=".urlencode($cptdomain);
                    ...
                    if ($dataattr['RowsCount'] > 0) {
                        ...
                        foreach ($datatables as $row) {
                            if (strtolower($row['Name']) == 'rootdomain') {
                                ...
                                echo $q . $qv;
                                //file_put_contents($mjcachepath . "a.sql", $q . $qv);
