<?php
/**
* Menus helper class file.
*
* Provides static functions for interaction with Menus from outside of the module.
*
...
class Menus
{
	...
	*/
	public static function getAssignedRoles($userId=null, $sort=true)
	{
		...
		if( $userId===null && $user->isGuest===false )
	 	...
	 	$authorizer = self::getAuthorizer();
	 	return $authorizer->getAuthItems(Menu::TYPE_TAB, $userId, null, $sort);
	}

	/**
	* Returns the base url to Menus.
	* @return the url to Menus.
	*/
	...
	*/
	...
	{
		return array(
			Menu::TYPE_OPERATION=>Menus::t('core', 'Operation'),
			Menu::TYPE_TASK=>Menus::t('core', 'Task'),
			Menu::TYPE_TAB=>Menus::t('core', 'Custom Menu'),
		);
	}
	...
	*/
	public static function getMenuTypeName($type)
	{
		...
		if( isset($options[ $type ])===true )
			...
			throw new CException(Menus::t('core', 'Invalid authorization item type.'));
	}
	...
	* @return string the authorization item type name.
	*/
	public static function getMenuTypeNamePlural($type)
	{
		switch( (int)$type )
		{
			case Menu::TYPE_OPERATION: return Menus::t('core', 'Operations');
			case Menu::TYPE_TASK: return Menus::t('core', 'Tasks');
			case Menu::TYPE_TAB: return Menus::t('core', 'Custom Menu');
			default: throw new CException(Menus::t('core', 'Invalid authorization item type.'));
		}
	}
	...
	*/
	public static function getMenuRoute($type)
	{
		switch( (int)$type )
		{
			case Menu::TYPE_OPERATION: return array('tab/menus/type/' . Menu::TYPE_OPERATION);
			case Menu::TYPE_TASK: return array('tab/menus/type/' . Menu::TYPE_TASK);
			case Menu::TYPE_TAB: return array('tab/menus/type/' . Menu::TYPE_TAB);
			default: throw new CException(Menus::t('core', 'Invalid authorization item type.'));
		}
	}
	...
	*/
	public static function getValidChildTypes($type)
	{
	 	switch( (int)$type )
		{
			...
			case Menu::TYPE_TAB: return array(Menu::TYPE_TAB, Menu::TYPE_TASK, Menu::TYPE_OPERATION);
			// Tasks can consist of other tasks and operations
			case Menu::TYPE_TASK: return array(Menu::TYPE_TASK, Menu::TYPE_OPERATION);
			// Operations can consist of other operations
			case Menu::TYPE_OPERATION: return array(Menu::TYPE_OPERATION);
			// Invalid type
			default: throw new CException(Menus::t('core', 'Invalid authorization item type.'));
		}
	}
	...
	*/
	public static function getMenuSelectOptions($type=null, $exclude=array())
	{
		$authorizer = self::getAuthorizer();
		$items = $authorizer->getAuthItems($type, null, null, true, $exclude);
		return self::generateMenuSelectOptions($items, $type);
	}
	...
	* @return array the select options.
	*/
	public static function getParentMenuSelectOptions(Menu $parent, $type=null, $exclude=array())
	{
		$items = Menus::getMenu()->getMenus($type, $parent, true, $exclude);
		return self::generateMenuSelectOptions($items, $type);
	}
	...
	* @return array the select options.
	*/
	protected static function generateMenuSelectOptions($items, $type)
	{
		$selectOptions = array();
		...
		// We have multiple types, nest the items under their types
       	if( $type!==(int)$type )
       	{
       		foreach( $items as $itemName=>$item )
				$selectOptions[ self::getMenuTypeNamePlural($item->type) ][ $itemName ] = $item->name;
		}
	...
	* showing Powered-by-Menus information.
	*/
	public static function powered()
	{
		$module = self::module();
		return 'Secured with <a href="http://www.yiiframework.com/extension/menus" rel="external">Menus</a> version '.$module->getVersion().'.';
	}

	/**
	* @return MenusModule the Menus module.
	*/
	...
	/**
	* Searches for the Menus module among all installed modules.
	...
	*/
	private static function findModule(CModule $module=null)
	{
		...
		if( ($m = $module->getModule('menus'))!==null )
			return $m;
	...
	*/
	...
	{
		return Yii::t('MenusModule.'.$category, $message, $params, $source, $language);
	}
}
