...
class SeoUtils {
    ...
    */
    ...
    public function getDomain($url) {
        $url = strtolower($url);
        if (stripos($url, ".") === 0) $url = substr($url, 1);
        ...
        if (preg_match('/(?P<domain>[a-z0-9][a-z0-9\-]{1,63}\.[a-z\.]{2,6})$/i', $domain, $regs)) {
            ...
            if (substr_count($dm, '.') >= 2) {
                $tld = substr(strrchr($dm, "."), 1);
                if (in_array($tld, array("aero","asia","biz","cat","com","coop","edu","gov","info","int",
                                         "jobs","mil","mobi","museum","name","net","org","pro","tel","travel","xxx"))) {
                    $_da = explode(".", $dm);
                    $_da = array_slice($_da, -2, 2);
                    //print_r($_da);
                    $dm = implode(".", $_da);
                }
            }
...
*/
    ...
    public function getDomainCreatedOn($domain) {
        ...
        $registry = @$html->find("div[id=registry_whois]", 0);
        if (!$registry) {
            return -99;
        }

        $importatdates = @$registry->find("div.important-dates", 0);
        if (!$importatdates) {
            $res = @$registry->plaintext;
            $res = str_replace("&nbsp;", " ", $res);
            if (preg_match('/(?P<name>Domain record activated):\s*(?P<onlinesince>[\w|-]+)/', $res, $matches)) {
                //print_r($matches);
                unset($res);
                unset($html);
                return trim($matches['onlinesince']);
            }
        } else {

            //###$res = @$html->find("div[id=registry_whois] div.important-dates table tr", 1)->childNodes(1)->plaintext;
            //$res = @$registry->find("div.important-dates table tr", 1)->childNodes(1)->plaintext;
            $res = @$importatdates->find("table tr", 1)->childNodes(1)->plaintext;
            unset($importatdates);
            unset($registry);
            unset($html);
            //echo $res;
    ...
    public function getAlexaDomainInfo($domain) {
        ...
        if( $result )
        {
            ...
            if ($a[1]) {
                ...
                if (empty($info['country']) && $b->COUNTRY) {
                    foreach($b->COUNTRY->attributes() as $k => $v) {
                        if ($k == 'CODE') {
                            $v = (string)$v;
                            $info['country'] = $v;
                        }
            ...
            if ($xml->DMOZ) {
                if ($xml->DMOZ->SITE->CATS) {
                    $cats = $xml->DMOZ->SITE->CATS;
                    foreach ($cats->children() as $cat) {
                        //print_r($v);
                        foreach ($cat->attributes() as $k => $v) {
                            if ($k == "ID") {
                                $v = (string)$v;
                                $c = explode("/", $v);
                                if ($c[0] == "Top") {
                                    if ($c[1] == "Recreation" && $c[2] == "Travel") $c[1] = "Travel";
                                    $info["cats"][] = $c[1];
                                } else {
                                    $info["cats"][] = $c[0];
                                }
                    ...
                    if (isset($info["cats"])) $info["cats"] = array_unique($info["cats"]);
                }
            }
        }
    ...
    public function getSubDomain($url){
        ...
        if ($domain && stripos($domain, ".") !== false) {
            $ds = explode(".", $domain);
            if ($ds[0] == "www" && count($ds) >= 3) unset($ds[0]);
            $domain = implode(".", $ds);
        } else {
            $domain = "";
        }
    ...
    ##10/24/2012
    //http://api.hostip.info/get_html.php?ip=202.101.172.35
    public function getCountryByIP($ip) {
        ...
        if ($result) {
            //cl means "country lable", fcn="Full Country name", scn="Short country name", ctl=City Lable, ipl=IP lable
            //if(preg_match('/(?P<cl>Country): (?P<fcn>[\w|-]+) \((?P<scn>[\w|-]+)\)\n/', $result, $match)) {
            if(preg_match('/(?P<cl>Country): (?P<fcn>[\s\S]+) \((?P<scn>[\w|-]+)\)\n(?P<ctl>City): (?P<city>[\s\S]+)\n(?P<ipl>IP): (?P<ip>[\d.]+)/', $result, $match)) {
                return $match;
            } else {
                //echo 'Failed to fetch alexa rank!';
                return false;
