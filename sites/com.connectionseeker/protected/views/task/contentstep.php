...
<h1>Content - Step<?php echo $content_step;?></h1>
		...
		array(
            ...
            'buttons' => array(
                ...
                'accept' => array(
                    ...
                    //'visible' => '($data->content_step == 1 || $data->content_step == 2)',
                    'visible' => '($data->content_step != 7)',
                ...
                'updatestepattr' => array(
                    'label' => 'Add Ideation',
                    'imageUrl' => $_themebaseurl.'/css/gridview/update.png',
                    'visible' => '($data->content_step == 3)',
                    'url' => 'Yii::app()->createUrl("task/view", array("id"=>$data->id))',
                ...
                'denywithreason' => array(
                    ...
                    'url' => 'Yii::app()->createUrl("task/setattr", array("id"=>$data->id,"attrname"=>"content_step","attrvalue"=>3))',
                    'visible' => '($data->content_step == 4) || ($data->content_step == 6)',
                    'options' => array(
                        'name'=>'stepreason',
                    ),
                ),
...
<?php } elseif($content_step == 4 || $content_step == 1) {
...
<?php if ($content_step >= 4 || $content_step == 2) :
if ($content_step == 2) {
    $_defaulttype = 2;
} else {
    $_defaulttype = 3;
...
$(document).ready(function() {
    ...
    $.fn.yiiGridView.defaults.afterAjaxUpdate = function(){
        ...
        $("a[name^='writernote'],a[name^='ideationnote'],a[name^='extranote'],a[name^='updatestepattr'],a[name^='steptitlehref']").each(function() {
            $(this).unbind('click').click(function(){
                ...
                if ($(this).attr("name") == "updatestepattr" || $(this).attr("name") == "steptitlehref") {
                    $("#writernoteboxdiv,#ideationnoteboxdiv,#extranoteboxdiv").hide();
                    $("#writernoteboxdiv,#ideationnoteboxdiv,#extranoteboxdiv").appendTo($("#hiddenContainer"));
