...
$('#addPrice').unbind('click').click(function(){
    if ($("#BloggerProgram_price").val() == "") {
        return false;
    }
    $.ajax({
        'success': function(data) {
            $("#domainPrice").append(data);
            $("#BloggerProgram_price").val("");
        },
        'type':'POST',
        'dataType': 'html',
        'url':"<?php echo Yii::app()->createUrl('/BloggerProgram/price', array('blogger_program_id' => $model->blogger_program_id));?>",
        'cache': false,
