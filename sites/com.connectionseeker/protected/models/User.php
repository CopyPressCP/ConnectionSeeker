...
class User extends CActiveRecord
{
	...
	public function rules()
	{
        ...
        */
        ...
        return array(
            ...
            array('email', 'email'),
            array('id, username, last_visit_time, email, created_by, client_id, status, authrole', 'safe', 'on'=>'search'),
        );
	}
    ...
    {
        $monthago = date("Y-m-d H:i:s", time() - 86400 * 30);//one month ago
        return array(
            'actived'=>array(
                'condition'=>'status=1',
            ),
            'csemployee'=>array(
                'condition'=>"rauthassignment.itemname NOT IN ('Marketer','Publisher')",
            ),
            'outreacher'=>array(
                'condition'=>"rauthassignment.itemname IN ('InternalOutreach','Outreach')",
            ),
            'marketer'=>array(
                'condition'=>"rauthassignment.itemname = 'Marketer'",
            ),
            'editor'=>array(
                'condition'=>"rauthassignment.itemname = 'Editor'",
            ),
            'csadmin'=>array(
                'condition'=>"rauthassignment.itemname = 'Admin'",
            ),
            //email reporting in month
            'emrinmonth'=>array(
                //'join' => "LEFT OUTER JOIN {{email_queue}} AS e ON (e.created_by = t.id)",
                'join' => "INNER JOIN {{email_queue}} AS e ON (e.created_by = t.id)",
                'condition'=>"(e.created >= '$monthago' AND (e.parent_id=0 OR (e.parent_id IS NULL)) AND (e.domain_id>0) AND (e.`from`>0) AND e.is_reply=0)",
                'group'=>"t.id",
            ),
        );
    }
	...
	public function search()
	{
		...
		$criteria->compare('t.display_mode',$this->display_mode);
		...
		$criteria->compare('t.last_visit_time',$this->last_visit_time,true);

        $criteria->with = array('rauthassignment');
        if ($this->authrole) {
            $criteria->compare('rauthassignment.itemname',$this->authrole);
        }

        $sort = new CSort();
        $sort->attributes = array(
            //'rauthassignment.itemname'=>array(
            'authrole'=>array(
                'asc'=>'rauthassignment.itemname ASC',
                'desc'=>'rauthassignment.itemname DESC',
            ),
