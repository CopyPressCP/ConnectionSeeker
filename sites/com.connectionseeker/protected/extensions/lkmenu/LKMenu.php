...
class LKMenu extends CWidget
{
   ...
   */
	...
	protected function renderMenuItem($item)
	{
		...
		{
            $name = $item['name'];
		return CHtml::link($name,$item['url'],isset($item['linkOptions']) ? $item['linkOptions'] : array());
		}
	}
	...
	{
        //added by leo 7/20/2012, Check the Request URL & the Tab Url, to see if it is the same or not!
        if (isset($_REQUEST["r"])) {
            $__r = strtolower($_REQUEST["r"]);
            if (strpos($__r, "/") !== 0) $__r = "/" . $__r;
        }
		...
		foreach($items as $i=>$item)
		{
			...
			if(!isset($item['active']))
			{
                ...
                //!!## plugin for highlight the parent tab. added by leo 7/20/2012, 
                //If the Request URL equal the Tab URL, that means this Tab will absolutely be highlight!!!
                if (isset($item['url']) && !empty($item['url'])) {
                    $__ir = strtolower($item['url'][0]);
                    if (strpos($__ir, "/") !== 0) $__ir = "/" . $__ir;
                    if (isset($__r) && $__r == $__ir) {
