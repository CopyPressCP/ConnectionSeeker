...
class ClientDomainController extends RController
{
	/**
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view', 'competitors', 'domains','create','update'),
				//'users'=>array('*'),
				'users'=>array('@'),
			),
    ...
    */
    ...
    public function actionDomains($client_id = 0)
    {
        ...
        //####################################################//
        //Plugin for the employee of the client! *******This Plugin had moved into the ClientDomain Model********
        /*
            ...
            if ($umodel->type == 0) {//owner or root!
                ...
                if ($umodel->duty_campaign_ids) {
                    ...
                    if ($_domain_ids) {
                        ...
                        $condition = " AND id IN (".implode(",", $_dids).")";
                        //echo $condition;
                        //print_r($_dids);
                        //echo implode(",", $_dids);
                    }
        ...
        */
        ...
        $term = trim($_GET['term']);
        if (!empty($term)) {
            $data = ClientDomain::model()->byduty()->findAll('client_id=:client_id AND domain LIKE :qterm'.$condition,
                          array(':client_id'=>(int)$client_id, ':qterm'=> '%'.$term.'%'));
        } else {
            $data = ClientDomain::model()->byduty()->findAll('client_id=:client_id'.$condition,
        ...
        if (!empty($data)) {
            $domains = "";
            foreach ($data as $p) {
                if (isset($_GET['format']) && strtolower($_GET['format']) == 'html4dropdown') {
                    $domains .= CHtml::tag('option',array('value'=>$p->id),CHtml::encode($p->domain),true);
                } else {
                    $rs[] = array(
            ...
            if (isset($_GET['format']) && strtolower($_GET['format']) == 'html4dropdown') $rs['domains'] = $domains;
        }
