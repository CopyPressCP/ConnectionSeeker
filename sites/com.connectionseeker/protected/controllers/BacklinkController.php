...
class BacklinkController extends RController
{
	/**
            ...
            foreach($ds as $k => $v) {
                ...
                if ($profile && isset($profile[$datasource])) {
                    ...
                    foreach($lables as $lk => $lv) {
                        ...
                        //we used the setCellValueExplicit to avoid the cell value which contain % raise error notice in the excel
                        if ($lk == "quality") {
                            $objPHPExcel->getActiveSheet()->setCellValueExplicit(Utils::columnName($a).$idx, $pf[$lk], PHPExcel_Cell_DataType::TYPE_NUMERIC);
                        } else {
