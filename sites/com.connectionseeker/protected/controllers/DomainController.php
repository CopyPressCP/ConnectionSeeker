...
class DomainController extends RController
{
    ...
    public function actionView($id)
    {
        if (Yii::app()->request->getQuery("ajax") == true) {
            $rs = array();
            $rs = Yii::app()->db->createCommand()->select("t.*, s.semrushkeywords AS semrushkeywords, s.mozrank")->from('{{domain}} t')
                    ->where('t.id=:id', array(':id'=>$id))
                    ->join('{{domain_summary}} s', 't.id = s.domain_id')
                    ->queryRow();
            $rs['creation'] = "-1";
            if ($rs['onlinesince'] == "-1" || $rs['onlinesince'] == "658454400") 
                $rs['creation'] = date("Y-m-d", $rs['onlinesince']);
            $rs["short_meta_keywords"] = mb_substr($rs["meta_keywords"], 0, 140);
            $rs["short_meta_description"] = mb_substr($rs["meta_description"], 0, 140);
            $rs["semrushkeywords"] = str_replace("|", ", ", $rs["semrushkeywords"]);

            $rs["lasttouchedby"] = ""; 
            if ($rs["touched_by"] > 0) {
                $_tbumodel = User::model()->findByPk($rs["touched_by"]);
                $rs["lasttouchedby"] = $_tbumodel->username;
            }

            $iors = Yii::app()->db->createCommand()->select("t.campaign_id, c.name")->from('{{inventory_building_task}} t')
                    //->where('t.desired_domain_id=:id  AND iostatus in (3,5,21)', array(':id'=>$id))
                    ->where('t.desired_domain_id=:id', array(':id'=>$id))
                    ->join('{{campaign}} c', 't.campaign_id = c.id')
                    ->group('t.campaign_id')
                    ->queryAll();
            $rs["used_campaigns"] = array();
            if ($iors) {
                $rs["used_campaigns"] = CHtml::listData($iors, 'campaign_id', 'name');
            }

            //########## START ADDED 7/10/2013, For buiding a relationship between domain and tierlevel ##########
            $tiers = Types::model()->actived()->bytype('tierlevel')->findAll();
            if ($tiers) {
                $outils = array();
                foreach ($tiers as $vt) {
                    $_search = array("googlepr","mozrank","alexarank");
                    $_replace = array('$rs["googlepr"]','$rs["mozrank"]','$rs["alexarank"]');
                    $_outil = str_ireplace($_search, $_replace, Utils::sqlCondition2Logic($vt->outils));
                    //eval only accepts statements, not expressions. So we need to convert the tests with: eval("return $_outil;");
                    if (eval("return $_outil;")) {
                        $_refid = ($vt->refid == 999) ? -$vt->refid : $vt->refid;
                        $outils[$_refid] = $vt->typename;
                    }
                }
                ksort($outils);
                //print_r($outils);
                $rs['tierlevel'] = array_pop($outils);
                //$rs['tierlevel'] = implode(",", array_values($outils));
            }
            //########## END ADDED 7/10/2013 For buiding a relationship between domain and tierlevel ##########

            //1/21/2014 added for company sites by email
            /*
            if (!empty($rs["email"])) {
                $cmpsites = Yii::app()->db->createCommand()->select("GROUP_CONCAT(domain separator ', ') AS sites")->from('{{domain}}')
                        ->where('email=:email', array(':email'=>$rs["email"]))->group("email")->queryRow();
                $rs["company_sites"] = $cmpsites["sites"];
            }
            */
    ...
    public function actionCreate()
    {
        ...
        if(isset($_POST['Domain']))
        {
            ...
            */
            //Re active the exist domains
            $domodel = $model->findByAttributes(array('domain'=>$_POST['Domain']['domain']));
            if ($domodel) {
                $model = $domodel;
            }
            $model->attributes=$_POST['Domain'];
     ...
     * 
     */
    public function actionEmail($domain=null)
    {
        if(!empty($domain)) {
            if (($pos = stripos($domain, '.')) === false) {
                throw new CHttpException(400,'Invalid domain. Please provide a valid domain.');
            }
            $url = $domain;
            ...
            $domain = SeoUtils::getDomain($url);
            $model=new Domain;
            //$domodel = Domain::model()->findByAttributes(array('domain'=>$domain));
            $domodel = $model->findByAttributes(array('domain'=>$domain));
            if (!$domodel) {
                ...
                $model->tld = substr(strrchr($domain, "."), 1);
                $model->save();
                $domodel = $model;
                unset($model);
            }
        } else {
            throw new CHttpException(400,'Invalid request. Please provide a domain.');
        }
    ...
    {
        //###Disable this funtion for now for some reason.
        throw new CHttpException(500,'Invalid request. Please contact the admin.');
        exit;
    ...
    public function actionSetattr($id)
    {
        ...
        if(isset($_GET['attrname']) && isset($_GET['attrvalue']))
        {
            ...
            if (stripos($_GET['attrname'], "stype") !== false) {
                ...
                if ($model->touched_status == 6 || $model->touched_status == 20) {
                    //if the user choose the site acquired,then add this domain into inventory table.
                    $model->triggerInventorySave = true;
                }
     ...
     * Manages all models.
     */
    public function actionBulkattr()
    {
        //if it is isAjaxRequest, then...
        if (Yii::app()->request->isAjaxRequest) {
            $rs['msg'] = "The operation completed successfully!";
            $rs['success'] = true;

            if (strtoupper($_POST["operation"]) == "MOVEALL") {
                parse_str($_POST["search"], $output);
                $_GET["Domain"]["excludecategory"] = $output["Domain"]["excludecategory"];
                $model=new Domain('search');
                $model->unsetAttributes();  // clear any default values
                if(isset($output["Domain"]))
                    $model->attributes=$output["Domain"];
                $criteria = $model->getSearchCriteria();

                $attributes['category'] = $_POST["category"];
                $attributes['category_str'] = $_POST["category_str"];

                $attr = $params = array();
                if ($criteria->params) $params = $criteria->params;
                ...
                //exit;
                if ($criteria->with) {
                    if ($criteria->with == "rsummary" || $criteria->with == array("rsummary")) {
                        //the updateAll method not support the table relationship, So we have to use createCommand
                        $q = "UPDATE {{domain}} t LEFT OUTER JOIN {{domain_summary}} rsummary ON (t.id=rsummary.domain_id)";
                    } else {
                        $rs['msg'] = "Unknown table relation, please contact admin.";
                    }
                } else {
                    //##updateAll not working on the t alias, so we have to replace t alias to ""!!
                    /*
                    if ($criteria->condition) $condition = preg_replace("/(\W)t\./i", "$1{{domain}}.", "(".$criteria->condition.")");
                    $nofu = Domain::model()->updateAll($attributes, $condition, $criteria->params);
                    */
                    $q = "UPDATE {{domain}} t";
                }
                if (isset($q) && $q) {
                    $q .= " SET ".implode(",", $attr);
                    $q .= " WHERE 1=1";
                    if (isset($criteria->condition) && $criteria->condition) $q .= " AND ".$criteria->condition;

                    $nofu = Yii::app()->db->createCommand($q)->bindValues($params)->execute();
                    $rs['msg'] = "The operation completed successfully! {$nofu} rows being updated.";
                }
                ...
                $domainarr = $_POST['Domain'];
                if (empty($domainarr['domain_ids']) || empty($domainarr['move2category'])) {
                    $rs['msg'] = "Please choose one domain first!";
                    $rs['success'] = false;
                } else {
                    $domains = explode(",", $domainarr['domain_ids']);
                    $rs["ids"] = $domains;
                    //##$categories = explode(",", $domainarr['move2category']);

                    foreach ($domains as $did) {
                        $domodel = Domain::model()->findByPK($did);
                        if ($domodel) {
                            $domodel->setIsNewRecord(false);
                            $domodel->setScenario('update');
                            $domodel->category = $domainarr['move2category'];
    ...
    */
    ...
    public function actionPrice($domain_id)
    {
        $model = new DomainPrice;

        if(isset($_POST['DomainPrice']))
        {
            $model->attributes=$_POST['DomainPrice'];
            if($model->save()) {
                // $this->redirect(array('view','id'=>$model->id));
            ...
            $domain_id = $_POST['DomainPrice']['domain_id'];
        } else if ($domain_id > 0) {
            $model->domain_id = $domain_id;
        }
        $data = $model->with('rcreatedby')->findAll('domain_id=' . $model->domain_id);
    ...
    */
    private function __addBatchTasksFromFile($p){
        ...
        */
        ...
        $probabilities = array_flip(Inventory::$probabilities);
        $probabilities = array_change_key_case($probabilities, CASE_UPPER);
...
//die();
        ...
        */
        $header = array("Domain"=>"domain","Site Type"=>"stype", "Outreach Type"=>"otype", "Contact Email"=>"email",
            "CS Category"=>"category_str","Category"=>"category_str","Primary Email"=>"primary_email","Contact Name"=>"owner",
            "Primary Email2"=>"primary_email2","Owner2"=>"owner2","URL"=>"domain",
            "PR"=>"googlepr","DOMAIN AUTH"=>"mozauthority","MOZRANK"=>"mozrank","EXT LINKS"=>"inboundlinks",
            "Technorati Category"=>"technorati_category_str","AWIS Category"=>"awis_category_str", "Status"=>"touched_status", "Price"=>"price","Probability"=>"probability");
            ...
            if (1 == $rowIndex) {
                ...
                foreach ($header as $hk => $hv) {
                    ...
                    if ($hv == 'domain') {
						...
						$price = $pv;
						continue;
                    } elseif ($hv == 'probability') {
                        //add to inventory pro
                        $pv = strtoupper($pv);
                        $pv = $probabilities[$pv];
						if (in_array($pv, array(1,2,3))) $probability = $pv;
						continue;
                        ...
                        if (empty($pv) && !empty($touched_status)) $pv = $touched_status;
                        if (empty($pv) && $pv !== 0) $pv = $touched_statuses["SITE UNTOUCHED"];
                    }

                    if (empty($pv) && ($hv != 'touched_status')) $pv = "";
                    if ($hv == 'stype' && empty($pv)) $pv = 0;
                    if (strtoupper($pv) == "NULL" && in_array($hv, array("primary_email","primary_email2","owner","owner2"))) {
                        $pv = "";
                    }
                    $p[$hv] = $pv;
                }
                ...
                if (in_array($action, array(1,2,3,4,5))) {
                    $p["status"] = 1;//set the domain active.
                    ...
                    if ($action == 5) {
                        $dmi = $model->with("rsummary")->findByAttributes(array('domain' => $p['domain']));
                    } else {
                    ...
                    if (!empty($dmi)) {
                        ...
                        */
                        if ($action == 4) {
                            //##The following line was added @2016-01-20
                            if (isset($p['touched_status']) && in_array($p['touched_status'], array(6,20)) ){
                                $dmi->triggerInventorySave = true;
                            }
                            //if ($dmi->status == 1) unset($p['touched_status']);
                            if (isset($p["category"]) && strlen($dmi->category)>0) {
                                if (is_numeric($p["category"])) {
                                    $dmi->category = $dmi->category."|".$p["category"];
                                    unset($p["category"]);
                                }
                                $_dcat = explode("|", $dmi->category);
                                foreach ($_dcat as $___dc) {
                                    if ($___dc) $p["category"][$___dc] = $___dc;
                                }
                            }
                            $dmi->setIsNewRecord(false);
                            $dmi->setScenario('update');
                            ...
                            //all other metrics do not get overwritten.  They only get added if we don't have that domain OR if that metrics is equal to null
                            //if ($action == 5) {
                            if ($action == 5 && $dmi->status == 1) {
                                if (isset($p["stype"]) && $dmi->stype != null) unset($p["stype"]);
                                if (isset($p["otype"]) && $dmi->otype != null) unset($p["otype"]);
                                if (isset($p["category"]) && strlen($dmi->category)>0) {
                                    if (is_numeric($p["category"])) {
                                        $dmi->category = $dmi->category."|".$p["category"];
                                        unset($p["category"]);
                                ...
                                if (isset($p["email"]) && $dmi->email != null) unset($p["email"]);
                                if (isset($p["primary_email2"]) && $dmi->primary_email2 != null) unset($p["primary_email2"]);
                                if (isset($p["owner"]) && $dmi->owner != null) unset($p["owner"]);
                                if (isset($p["owner2"]) && $dmi->owner2 != null) unset($p["owner2"]);
                                if (isset($p["googlepr"]) && $dmi->googlepr != -1) unset($p["googlepr"]);
                                if (isset($p["mozauthority"]) && $dmi->rsummary->mozauthority != 0) 
                                    unset($p["mozauthority"]);
                                if (isset($p["mozrank"]) && $dmi->rsummary->mozrank != 0) unset($p["mozrank"]);
                                if (isset($p["inboundlinks"]) && $dmi->inboundlinks != 0) unset($p["inboundlinks"]);
                                if (isset($p["touched_status"]) && $dmi->touched_status != 0) unset($p["touched_status"]);
                            }//######end if action == 5;

                            //####if ($dmi->touched_status == 0) {
                            if ($dmi->status == 1) {
                                if ($dmi->touched_status <= 1 || $action == 5) {
                                    $dmi->setIsNewRecord(false);
                                    $dmi->setScenario('update');

                                    $isnew = true;
                                    if (isset($p['touched_status']) && 
                                           ( ($p['touched_status'] == 6 && !in_array($dmi->touched_status, array(6,20)) ) 
                                          || ($p['touched_status'] == 20 && $dmi->touched_status != 20) ) ) {
                                        $dmi->triggerInventorySave = true;
                                    } else {
                                        if (isset($p['touched_status']) && in_array($dmi->touched_status, array(6,20))) {
                                            unset($p['touched_status']);
                                        }
                                    }
                        ...
                        $dmi->id=NULL;
                        $dmi->attributes=$p;
                        if (in_array($dmi->touched_status, array(6,20))) {
                            $dmi->triggerInventorySave = true;
                        }
                        $_succss = $dmi->save();
                    }
					...
					//add domain price into tbl.domain_price
					if ((!empty($price) || !empty($probability)) && $p['domain']) {
                        $dmi = $model->findByAttributes(array('domain' => $p['domain']));
						if ($dmi) {
                            if (!empty($price)) {
                                $p = array();
                                ...
                                $p['domain'] = $dmi->domain;
                                $p['price'] = $price;
                                if (!isset($dpmdl)) $dpmdl = new DomainPrice;
                                $dpricemdl = $dpmdl->findByAttributes(array('domain_id' => $p['domain_id']));
                                if (!empty($dpricemdl)) {
                                    $dpricemdl->setIsNewRecord(false);
                                    $dpricemdl->setScenario('update');
                                } else {
                                    $dpricemdl = $dpmdl;
                                    $dpricemdl->setIsNewRecord(true);
                                    $dpricemdl->id=NULL;
                                }

                                $dpricemdl->attributes=$p;
                                $dpricemdl->save();
                                unset($dpricemdl->attributes);
                                unset($dpricemdl);
                                unset($price);
                            }

                            //add inventory Probability into tbl.inventory
                            if (!empty($probability)) {
                                if (!isset($ivtmdl)) $ivtmdl = new Inventory;
                                $dpromdl = $ivtmdl->findByAttributes(array('domain_id' => $dmi->id));
                                if (!empty($dpromdl)) {
                                    $dpromdl->setIsNewRecord(false);
                                    $dpromdl->setScenario('update');
                                    $dpromdl->probability = $probability;
                                    $dpromdl->triggerDomainSave = false;
                                    $dpromdl->save();
                                    unset($dpromdl->attributes);
                                    unset($dpromdl);
                                    unset($probability);
                                }
                            }
