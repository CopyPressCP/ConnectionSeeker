...
class BloggerProgramNoteController extends RController
{
	/**
    ...
    */
	...
	public function actionIndex()
	{
		...
		$model=new BloggerProgramNote('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['BloggerProgramNote']))
			$model->attributes=$_GET['BloggerProgramNote'];
	...
	{
		$model=BloggerProgramNote::model()->findByPk($id);
		if($model===null)
	...
	public function actionIcon()
	{
        ...
        if (empty($ids)) {
            ...
            $notes = BloggerProgramNote::model()->findAll($criteria);
            if ($notes) {
                $currenttime = time();
                foreach ($notes as $k => $v) {
                    ...
                    */
                    ...
                    if ( ($currenttime - $freshcreated)<=86400 ) {
                        $rs["freshnote"][$v->blogger_program_id] = $v->blogger_program_id;
                    }
                }
            }
        }
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='blogger-program-note-form')
		{
