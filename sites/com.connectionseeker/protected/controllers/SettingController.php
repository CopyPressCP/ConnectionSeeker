...
class SettingController extends RController
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform the following actions
				'actions'=>array('profile','mailer','template','createMailer','updateMailer','createTemplate','updateTemplate','viewMailer','viewTemplate','deleteMailer','deleteTemplate'),
				'users'=>array('@'),
			),
            /*
	...
	public function actionViewTemplate($id)
	{
        ...
        if (isset($roles["Outreach"]) || isset($roles["InternalOutreach"])) {
            ...
            if ($cuid != $model->created_by) {
                throw new CHttpException(403,'You have no permission view this template item.');
            }

            $this->render('/template/view',array(
                'model'=>$model,
            ));
        } else {
            //redirect it to the Template managemant
	...
	public function actionViewMailer($id)
	{
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
        $cuid = Yii::app()->user->id;
        ...
        if (isset($roles["Outreach"]) || isset($roles["InternalOutreach"])) {
            ...
            if ($cuid != $model->created_by) {
                throw new CHttpException(403,'You have no permission view this mailer item.');
            }

            $this->render('/mailer/view',array(
                'model'=>$model,
            ));
        } else {
            //redirect it to the Template managemant
	...
	public function actionDeleteTemplate($id)
	{
        ...
        if (isset($roles["Outreach"]) || isset($roles["InternalOutreach"])) {
            if(Yii::app()->request->isPostRequest) {
                $model=Template::model()->findByPk($id);
                if ($cuid != $model->created_by) {
                    throw new CHttpException(403,'You have no permission delete this item.');
                    Yii::app()->end();
                }
                $model->status = 0;
                $model->save();
	...
	public function actionDeleteMailer($id)
	{
        $cuid = Yii::app()->user->id;
        $roles = Yii::app()->authManager->getRoles($cuid);
