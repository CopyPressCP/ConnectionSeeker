    ...
    public function run($args) {
        ...
        foreach($rules as $r) {
            ...
            if (!empty($r["days"])) {
                ...
                if (!in_array($week, $_weeks)) {
                    echo "Start--->".$r["id"]."----->End";
                    continue;}
            }

            //Check the time is between time start & time end.
            echo "|" . $timestartstamp = strtotime($r["time_start"]);
            ...
            if (filter_var($domainobj->primary_email, FILTER_VALIDATE_EMAIL)) {
                ...
                //$domainobj->save();
                Yii::app()->db->createCommand()->update('{{domain}}', array("primary_email"=>NULL,"modified"=>$now),
                                                'id=:id', array(':id'=>$current_domain_id));
                Yii::app()->db->createCommand()->update('{{automation_setting}}', array("domain_queue" => NULL,"modified"=>$now),
                                            'id=:id', array(':id'=>$r["id"]));
                continue;
            }
            ...
            if($rs) {
                ...
                if ($c) {
                    ...
                    $attrs = array();
                    if ($domainobj->touched_status == 11) {
                        $attrs['touched_status'] = 19;
                    } else {
                        if (!in_array($domainobj->touched_status, array(6,20,11,19,18))) {
...
function genDomainQueue($r) {
    ...
    if (isset($r["stype"]) && !empty($r["stype"])) {
        $site_type = str_replace("|", ",", $r["stype"]);
        $where .= " AND t.stype IN (".$site_type.")";
    }

    if (isset($r["otype"]) && !empty($r["otype"])) {
        $outreach_type = str_replace("|", ",", $r["otype"]);
        $where .= " AND t.otype IN (".$outreach_type.")";
    }
