            ...
            foreach ($domains as $dv) {
                ...
                if ($metas) {
                    if (isset($metas["keywords"]) && !empty($metas["keywords"])) {
                        $charset = Utils::mbCheckEncoding($metas["keywords"]);
                        $dv["meta_keywords"] = @mb_convert_encoding(trim($metas["keywords"]), "UTF-8", $charset);
                        $dv["meta_keywords"] = @mb_substr($dv["meta_keywords"], 0, 1000);
                    }
                    if (isset($metas["description"]) && !empty($metas["description"])) {
                        $charset = Utils::mbCheckEncoding($metas["description"]);
                        $dv["meta_description"] = @mb_convert_encoding(trim($metas["description"]), "UTF-8", $charset);
